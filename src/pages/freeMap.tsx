...
const FreeMap = () => {
  ...
  const handleClick = () => {
    ...
    switch (coordinates.step) {
      case 0:
        setCoordinates({
          startLocation: newCenter.reverse().join(","),
          step: 1,
        });
        break;
      case 1:
        setCoordinates({
          destination: newCenter.reverse().join(","),
          step: 2,
        });
        break;
      case 2:
    ...
    <div className="w-full h-dvh relative ">
      {routeIsFetching && (
        <div className="fixed flex flex-col gap-5 inset-0 bg-gray-600/50 z-50 pt-10 text-center text-white">
          <Spinner size={50} color="text-purple-700" />
      ...
      <div className="  absolute bg-white shadow-2xl rounded-lg p-4 container bottom-0 right-0 left-0 z-30 mx-auto">
        {coordinates.step !== 3 ? (
          <>
            ...
            <button
              className={` p-1 text-white  rounded-lg mt-2 h-12 w-full focus:shadow-md hover:opacity-85 bg-purple-700 disabled:bg-gray-400`}
              disabled={isFetching}
              onClick={handleClick}
            >
              {isFetching ? (
